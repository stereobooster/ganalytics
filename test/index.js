...
const resetLocalStorage = () => (global.localStorage = {});

function isData(t, tid, evt, obj = {}) {
	let src = _SENT_.src;
	t.ok(src, "~> used an Image and set `src` attribute");
	t.true(
		src.startsWith("https://www.google-analytics.com/collect?v=1"),
		"~~> sent to Google-Analytics API"
	);
	t.true(
		src.includes(`&tid=${tid}`),
		"~~> includes the UA idenfitifer (`tid`)"
	);
	t.true(
		src.includes(`&t=${evt}`),
		`~~> includes the event type: "${evt}" (\`t\`)`
	);
	t.true(
		src.includes(`&cid=${localStorage[KEY]}`),
		"~~> includes the generated user-idenfitifer (`cid`)"
	);
	t.true(
		src.includes("&z="),
		"~~> includes unique timestamp (`z` cache-buster)"
	);
	for (let k in obj) {
		t.true(
			src.includes(`&${k}=${encodeURIComponent(obj[k])}`),
			`~~> includes the encoded(?) \`${k}\` value`
		);
	}
}
...
test(`localStorage['${KEY}']`, t => {
	...
	let baz = new GA("", { consent: true });
	t.not(localStorage[KEY], uid, `generated unique "${KEY}" value`);
	t.is(
		baz.args.cid,
		localStorage[KEY],
		"~> GA instance keeps a copy as `args.cid` key"
	);
...
test("ga.send :: toWait", t => {
	...
	let ctx = GA("UA-STRING", { consent: true }, true);
...
test("ga.send", t => {
	...
	let foo = GA("UA-STRING", { consent: true }, true);
	...
	t.false(
		_SENT_.src.includes(`&dt=`),
		"~~> does NOT auto-include the `document.title` when options are passed (`dt`)"
	);
	t.false(
		_SENT_.src.includes(`&dl=`),
		"~~> does NOT auto-include the `location.href` when options are passed (`dl`)"
	);
